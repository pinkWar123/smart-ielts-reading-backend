...
class QuestionNotFoundError(Error):
    def __init__(self, question_id: str):
        super().__init__(f"Question with ID {question_id} not found", ErrorCode.NOT_FOUND)


class QuestionGroupNotFoundError(Error):
    def __init__(self, group_id: str):
        super().__init__(f"Question group with ID {group_id} not found", ErrorCode.NOT_FOUND)
...
class DuplicateQuestionGroupOrderError(Error):
    def __init__(self, order: int):
        super().__init__(
            f"Question group with order {order} already exists",
            ErrorCode.CONFLICT
        )
